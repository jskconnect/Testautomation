...
public class NewsPage {
	...
	@FindBy(xpath = "//span[contains(text(), 'Laura Muir keeps faith and is rewarded with Olympic')]")
	...
	public void confirmNewsValid() throws Exception {
		...
		// Search News in google page - PatternMatch
		Utilities.isTextMatches(globalmap.get("newstoSearch"));

		// Search News in google page - selenium Element Match
		scom.searchElementsInPage(globalmap.get("newstoSearch"), "silver");
	...
	public void verifyNewsMessage(String fieldname, List<List<String>> news) throws Exception {
		System.out.println("datatable list value is :" + news.get(0).get(0));
		String strsearchnews = null;
		GooglePage gp = new GooglePage();
		WebElement wenews = Driver.getDriverInstance().findElement(By.xpath("//span[contains(text(), '"+news.get(0).get(0)+"')]"));
		SeleniumCommon scom = new SeleniumCommon();
		...
		// Search News in google page - selenium Element Match
		scom.searchElementsInPage(globalmap.get("newstoSearch"), fieldname);
